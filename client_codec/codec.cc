...
void ProtobufCodec::defaultErrorCallback(
		const muduo::net::websocket::WebsocketConnectionPtr& conn,
...
void ProtobufCodec::onMessage(
		const muduo::net::websocket::WebsocketConnectionPtr& conn,
		const char* data, size_t len)
{
	Buffer buf;
	buf.append(data, len);
	if (buf.readableBytes() >= kMinMessageLen + kHeaderLen)
	{
    const int32_t len = buf.peekInt32();
    if (len > kMaxMessageLen || len < kMinMessageLen)
    {
      errorCallback_(conn, buf.peek(), buf.readableBytes(), kInvalidLength);
    }
    else if (buf.readableBytes() >= implicit_cast<size_t>(len + kHeaderLen))
    {
      ...
      {
        errorCallback_(conn, buf.peek(), buf.readableBytes(), errorCode);
      }
    }
	}
	else
