...
class ProtobufCodec : muduo::noncopyable
{
 public:
  ...
  typedef std::function<void (const muduo::net::websocket::WebsocketConnectionPtr&,
  ...
  void onMessage(const muduo::net::websocket::WebsocketConnectionPtr& conn,
                 const char* data, size_t len);

  void send(const muduo::net::websocket::WebsocketConnectionPtr& conn,
            const google::protobuf::Message& message)
  {
    muduo::net::Buffer buf;
    fillEmptyBuffer(&buf, message);
		conn->sendBinary(buf.peek(), buf.readableBytes());
  }
 ...
 private:
  static void defaultErrorCallback(const muduo::net::websocket::WebsocketConnectionPtr&,
																	 const char*, size_t len,
																	 ErrorCode);
