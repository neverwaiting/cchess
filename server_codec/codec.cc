...
void ProtobufCodec::defaultErrorCallback(int id, const muduo::net::TcpConnectionPtr& conn,
                                         ...
                                         ErrorCode errorCode)
{
  ...
  {
		LOG_INFO << "error protobuf message in codec";
    //conn->shutdown();
  }
}
...
{
  while (buf->readableBytes() >= kMinMessageLen + kHeaderLen + kIdLen)
  {
		const int16_t id = buf->peekInt16();
		const int32_t len = ntohl(static_cast<uint32_t>(*(uint32_t*)(buf->peek() + 2)));
    if (len > kMaxMessageLen || len < kMinMessageLen)
    {
      errorCallback_(id, conn, buf, receiveTime, kInvalidLength);
      break;
    }
    else if (buf->readableBytes() >= implicit_cast<size_t>(len + kHeaderLen + kIdLen))
    {
      ErrorCode errorCode = kNoError;
      MessagePtr message = parse(buf->peek()+kHeaderLen+kIdLen, len, &errorCode);
      if (errorCode == kNoError && message)
      {
        messageCallback_(id, conn, message, receiveTime);
        buf->retrieve(kIdLen+kHeaderLen+len);
      }
      else
      {
        errorCallback_(id, conn, buf, receiveTime, errorCode);
        break;
      }
    }
    else
